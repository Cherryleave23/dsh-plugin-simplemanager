...
export type StatusDTO = {
  ...
  present: boolean
  /** 运行阶段/来源归类。third 第三方 + 会话热装=temporary、持久装配=persistent、残留=orphan。 */
  phase: 'running' | 'failed' | 'temporary' | 'persistent' | 'orphan' | 'absent'
...
export function registerAgentTools(ctx: Context, ops: AgentOps): (() => void)[] {
  ...
  reg(
    defineTool({
      name: 'pm_tempLoad',
      description:
        '临时热装插件到运行时（不写入持久装配），输入 spec（registry 包名/本地路径/file:）。同名二次热装返回名可能带 -hotN，属正常。成功后用返回的 packageName 接续后续操作。',
